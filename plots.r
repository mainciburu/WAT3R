...
barcodes<-read.csv(paste0(mysample, "_barcode_results.csv"))

bc.sc<-read.table(scRNAannotation, header = T)
bc.sc$V1<-substr(x = bc.sc$V1, start = 1, stop = 16)
table(bc.sc$V3[bc.sc$V1%in%barcodes$BC])
...
tt<-prop.table(table(bc.sc$V3, bc.sc$TCR), margin = 1)
df<-melt(tt)
...
pheatmap::pheatmap(tt, color = col,
                   width = 8, height = 9,
                   fontsize = 10,
                   filename = "./plots/CDR3_clones.pdf")
...
TRB.lgd <- Legend(at = as.character(unique(heatmap.tib$TRB_CDR3)),
                  legend_gp = gpar(fill = cols[as.character(unique(heatmap.tib$TRB_CDR3))]), title = "TRB variable region")
